...
function main_program()
    ...
    size_grad = 20*factor; 
    mag = 5.6;
    diff_rate = 0.05*factor^2; %diffusion coefficient for Green's function (concentration)      
    fixed_time = 2000/factor;
    ...
    for i =1:N_cell
      b = N_IB*(i-1);
      %assume that p(2+b,:) is the location of receptor cluster on each cell
    ...
    for i = 1:total_time_steps
        ...
        if(mod(i-1,N_step) == 0) 
            ...
            draw_Green_gradient(Max,Xs,Ys,diff_rate,size_grad,fixed_time,mag,factor) %Draw attractant gradient
            hold on
...
function draw_Green_gradient(Max,Xs,Ys,D,size_grad,t,mag,factor)
    scale = 0.5*factor;
    [x,y] = meshgrid(-mag*size_grad:scale:mag*size_grad,-mag*size_grad:scale:mag*size_grad);
    r2=(x-Xs).^2+(y-Ys).^2;
    G=Max*exp(-r2/(4*D*t))/(4*pi*D*t); 

    [Cc,hc] = contour(x,y,G,10);  
