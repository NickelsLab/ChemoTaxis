...
EpuckDriver::EpuckDriver(ConfigFile* cf, int section)
  ...
  ,EXPECTED_EPUCK_SIDE_VERSION(122) // Version 1.2.2
{
  std::string strSerialPort(cf->ReadString(section, "port", "/dev/rfcomm3"));
  ...
  PLAYER_MSG0(1,"Driver data structure set up");
}
...
{
  PLAYER_MSG0(1,"Initializing Serial Port");
  ...
  unsigned int epuckVer = 0;
  while (epuckVer != this->EXPECTED_EPUCK_SIDE_VERSION) {
	  char buff[256], month[12];
	  int nr,v1,v2,v3,yr;
	  PLAYER_MSG1(1,"Checking E-puck firmware version",0);
	  this->serialPort->sendCharArray((char *)"v\n",2);
	  this->serialPort->recvCharArray(buff,256);
	  // keep sending version requests....
	  if (buff[0] != 'v') {
		  PLAYER_WARN("Re-requesting firmware version....");
		  continue;
	  }
	  nr=sscanf(buff,"v,Version %d.%d.%d %s %d GCtronic\n",
			  &v1,&v2,&v3,month,&yr);
	  if (nr>3) {
		  epuckVer = v1*100+v2*10+v3;
	  }
	  this->serialPort->recvCharArray(buff,256); // throw away second line

	  //if (epuckVer != this->EXPECTED_EPUCK_SIDE_VERSION)
	  //{
	    PLAYER_MSG1(1,"E-puck firmware version: %d", epuckVer);
	    PLAYER_MSG1(1,"Expected E-puck firmware version: %d", this->EXPECTED_EPUCK_SIDE_VERSION);
	    //PLAYER_ERROR("The e-puck side program version isn't the expected");
	    //return -1;
	  //}
  }

  PLAYER_MSG0(1,"NOT Initializing EPuck Camera");
  /*
  PLAYER_MSG0(1,"Initializing EPuck Camera");
  ...
  */
  ...
  PLAYER_MSG0(1,"Calibrating IR Sensors");
  char buff[256];
