    ...
    for iter=1:max_it  
        ...
        if(iter >= it_coll)
            ...
            for i=1:num_coll_par
                ...
                concinterp = griddata(xgrid,ygrid,usv, rc(i,1), rc(i,2));
                %current_asp = 10;
...
%     cinc = (cmax - cmin) / 50;
%     clevs = cmin:cinc:cmax;
    z = griddata(xgrid,ygrid,usv, xxxg,yyyg);

    [~,hc] = contour(xxxg,yyyg,z,0:0.1:500);  %replaced Cc with ~ to stop warning
    colorbar('location','eastoutside','FontSize',fsize)
    set (hc,'LineWidth', 3);
    axis equal
