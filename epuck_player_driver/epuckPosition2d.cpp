...
//     PLAYER_WARN("Rotational speed from epuck motor saturated in maximum value");
  ...
  */
  ...
  sprintf(cmd,"d,%d,%d\n",stepsR,stepsL);
  this->serialPort->sendCharArray(cmd,strlen(cmd));
  this->serialPort->recvCharArray(resp,255); // receive response
}
...
EpuckPosition2d::SetOdometry(Triple odometry)
{
  ...
  this->SendRequest(EpuckInterface::RST_STEPS);
...
EpuckPosition2d::UpdateOdometry()
{
  ...
  if (sscanf(buff,"q,%d,%d\n",&steps_right,&steps_left)!=2) {
		PLAYER_ERROR1("Error parsing '%s' to get R/L steps\n",buff);
  }
