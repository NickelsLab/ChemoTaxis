function [ ln_ic k_iter ] = intraparticlenodes(x_center,y_center,radius)
...
%      i=ix_left:ix_right;
%      j=iy_down:iy_up;
%      [dx_c dy_c]=meshgrid(i-x_center,j-y_center);
%      distance=sqrt((dx_c.*dx_c)+(dy_c.*dy_c));
%      i_grid=repmat(i,size(j,2),1);
%      j_grid=repmat(j,size(i,2),1)';
%      ln_ic=[i_grid(distance<=radius) j_grid(distance<=radius)];
%      k_iter=nnz(distance<=radius);
