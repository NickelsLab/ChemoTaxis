...
EpuckIR::IRData EpuckIR::GetIRData() const
{
  ...
  while (buff[0]!='n') {
	  ...
	  this->serialPort->recvCharArray(buff,256); // receive response "n,s0,s1,...,s7"
  }
  ...
  for(unsigned sensor = 0;  sensor < EpuckIR::SENSOR_QUANTITY; ++sensor)
  {
	...
	nr=sscanf(ptr,",%d",&reading);
	
	if (nr !=1)
		PLAYER_ERROR2("Error parsing '%s' to get IR sensor %d\n",
				buff,sensor);
	//else 
	//	PLAYER_MSG2(1,"IR[%d]=%d",sensor,reading);
