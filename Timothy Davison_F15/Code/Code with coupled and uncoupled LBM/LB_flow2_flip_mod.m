...
for test = 1:1:1
    ...
    fac_tumble=5;
    fac_mb = 0.85;
    max_it= 500;                                                                %maximum number of time step (in LB)
...
%     cent = [lx-10  ly-10];
    ...
    for i=1:num_coll_par
        [current_asp,~] = real_Berg_gradient(Max,Xs-200,Ys,diff_rate,rc(i,:),fixed_time);
        %current_asp=10;
    ...
    [ca,~] = real_Berg_gradient(Max,Xs-200,Ys,diff_rate,Xg,fixed_time);
    ...
    for iter=1:max_it  
        ...
        if(iter >= it_coll)
            ...
            for i=1:num_coll_par
                ...
                [current_asp(i),~] = real_Berg_gradient(Max,Xs-200,Ys,diff_rate,rc(i,:),fixed_time);
                %current_asp = 10;
                [m(i),mb(i),cheYp(i),~] = rapid_cell_1(current_asp(i),m(i),delT);
                RB = mb(i);
                %RB = 1;
                %RB = 0;
