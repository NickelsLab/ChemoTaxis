/* Copyright 2015 Kevin Nickels <knickels@trinity.edu>
 *
...
#include "epuckFloor.hpp"

EpuckFloor::EpuckFloor(const SerialPort* const serialPort)
...
EpuckFloor::FloorData
EpuckFloor::GetFloorData() const
{
  unsigned int reading;
  FloorData irData;
  this->SendRequest(EpuckInterface::GET_Floor_PROX);

  char buff[256];
  Port->recvCharArray(buff,256); // receive response "n,s0,s1,...,s7"
  unsigned int reading[EpuckFloor::SENSOR_QUANTITY];
  char *ptr = buff+1; //start with ,s0

  for(unsigned sensor = 0;  sensor < EpuckFloor::SENSOR_QUANTITY; ++sensor)
  {
    //reading = this->serialPort->recvInt();
	if (sscanf(ptr,",%d",&reading) !=1)
		PLAYER_ERROR2(1,"Error parsing '%s' to get Floor sensor %d\n",
				buff,sensor);
